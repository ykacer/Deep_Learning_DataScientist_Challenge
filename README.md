...
This page challenges many issues and pitfalls one can face when training/predicting models for vehicles and aircrafts localization in satellite images.

In a first part, we propose tricks and improvments for an aircrafts detection model on high resolution satellites images, according to some last training feedbacks and inference latency time constraint.
...
In a last part, we are called to propose a python code to post-process a segmentation mask failing at separating some vehicles and fully locating some single vehicles. The objective behind is to have the right count of vehicles present into the image.

Those three problems are assessed in the following sections

## I/ Improve the model, please, it’s not good enough

The product manager asks you to improve an aircraft detector in order to reach very good performances for the product. Here are some feedbacks regarding the last successful training.
...
- *small objects detection*
  ...
  Instead of relying only on backbone last convolution map (where small objects are likely to disappear due to successives max-poolings), RetinaNet also takes into account intermediate low level feature maps (FPN) to get chance of classifying and localing small objects

- *class imbalancing*

  RetinaNet extends classical cross entropy loss by multiplying the class log-probability with a specific term called "focal" term. This term is supposed to lower the loss contribution when probability is close to 1 (which likely correspond to "easy" background majority class examples). This factor comes with a positive integer exponent to control how much you need to counter balance background "easy" examples. The following figure from original paper well illustrates factor impact:
...
- *Inference-only tricks*
  ...
  They are quick-wins because they are fast to experiment so you can iterate a lot until reaching better performance

- *Training hyperparameters tuning*

  This process is straightforward and automatic to experiment thanks to cross-validation
...
- *Architecture/weight modifications*
  ...
  There are very challenging because they can change metrics a lot (and sometimes resulting in overfitting for example) and one could need many iterations to figure out a set of hyperparameters that enhances the whole performance
...
* Different learning rate strategies can be experimented like SGD used in original RetinaPaper

* Batch size can be increased for more stable convergence, but always constrained by GPU memory capacity limit

* the focal loss factor parameter (*gamma*) can be increased to more focus on foreground loss, then improving both precision and recall
...
* There is no information about metrics, but we can use *Mean Average Precision* (mAP) as validation metric to early stop training and avoid overfitting. The *Mean* here is expected to be a mean over different IOU thresholds (typically from 0.5 to 0.95 with 0.05 step) but one can change the range regarding the business expectations (if very precise localizations are needed due to critical situations, the range should be shifted to higher values)
Additionally, we can monitor mAPs, mAPm and mAPl metrics to assess how model performs on small, medium and large targets respectively, provided that it makes sense regarding our aerial images.


### I.3/ Object-detection hyperparameters tuning

- as stated in the subsection dedicated to inference-only tricks, we can also adapt and limit sizes/ratios proposals early at training stage to fit with ground truth boxes distribution and make training more efficient.
...
* Backbone Resnet50 can be progressively replaced by Resnet101 and then resnet 152 while carrefully assessed that you are not overfitting (hopefully, our data size is in the order of magnitude of CIFAR used by Resnet authors). TFLOPS operations increasing is linear so we should still be under the 5 mins budget (see below a schema from the original [paper](https://arxiv.org/abs/1512.03385):
...
It could be interesting to use Watershed Segmentation to do a finer and fast segmentation. The connected component could be eroded a little bit and used as marker for the Watershaed.
